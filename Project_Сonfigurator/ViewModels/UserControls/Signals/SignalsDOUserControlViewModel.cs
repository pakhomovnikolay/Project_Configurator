...
namespace Project_Сonfigurator.ViewModels.UserControls.Signals
{
    public class SignalsDOUserControlViewModel : ViewModelUserControl
    {
        ...
        public SignalsDOUserControlViewModel(IUserDialogService _UserDialog, ISignalService _ISignalService) : this()
        {
        ...
        #region Коллекция сигналов DO для отображения
        /// <summary>
        /// Коллекция сигналов DO для отображения
        /// </summary>
        ...
        private void OnCmdGeneratedTableExecuted()
        {
            ...
            var _Params = new ObservableCollection<SignalDO>();
                        ...
                        if (_Module.Type == TypeModule.DO)
                        {
                            ...
                            foreach (var Channel in _Module.Channels)
                            {
                                ...
                                if (int.TryParse(Channel.Address, out int address))
                                    Address = address - 200000;

                                var param = new SignalDO
                                {
                                    Signal = new BaseSignal
                                    {
                                        ...
                                        Area = "",
                                        Address = $"{Address}",
                                        VarName = $"do_shared[{_Params.Count + 1}]",
            ...
            Params = new ObservableCollection<SignalDO>(_Params);
            UserDialog.SendMessage("Импорт сигналов DO", "Сигналы DO успешно импортированы\nиз таблицы сигналов",
        ...
        private void OnCmdChangeAddressSignalExecuted(object p)
        {
            ...
            DoSelection = true;
            var NameListSelected = SignalServices.ChangeAddress(Title, TypeAddress.DO, SelectedParam.Signal.Area);
        ...
        #region Фильтрация сигналов DO
        /// <summary>
        /// Фильтрация сигналов DO
        /// </summary>
        public void ParamsFiltered(object sender, FilterEventArgs e)
        {
            #region Проверки до начала фильтрации
            // Выходим, если источник события не имеет нужный нам тип фильтрации, фильтр не установлен
            if (e.Item is not SignalDO _Param || _Param is null) { e.Accepted = false; return; }
            ...
            while (Params.Count < 2500)
            {
                var param = new SignalDO
                {
                    Signal = new BaseSignal
                    {
                        ...
                        Area = "",
                        Address = "",
                        VarName = $"do_shared[{Params.Count + 1}]",
                        LinkValue = ""
                    }
                };
                Params.Add(param);
