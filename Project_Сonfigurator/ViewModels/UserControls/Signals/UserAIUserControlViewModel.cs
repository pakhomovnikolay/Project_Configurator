...
{
    public class UserAIUserControlViewModel : ViewModelUserControl
    {
        #region Конструктор
        ...
        public UserAIUserControlViewModel(ISignalService _ISignalService) : this()
        {
        ...
        private void CreateData()
        {
            while (Params.Count < 1200)
            {
                var param = new BaseSignal
                {
                    ...
                    VarName = $"user_ai[{Params.Count + 1}]",
                    LinkValue = ""
                };
